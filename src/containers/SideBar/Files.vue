    ...
    >
      <ul class="list-container">
        <li
          :style="{
            width: '100%',
            height: `${LINE_HEIGHT - reviseOffset}px`,
          }"
        />
        <FileTreeItem
...
import { ICacheTree, ILogicTree, ISideBarState } from "@/typings/modules/sideBar";
...
export default class Files extends Vue {
   ...
   */
  get logicalHeight() {
    return this.fileList.length * this.LINE_HEIGHT;
  }
   ...
   */
  get visibleCount() {
    return Math.floor(this.visibleHeight / this.LINE_HEIGHT) + 1;
  }
  ...
  handleScroll(value: MouseWheelEvent) {
    ...
    if (this.invisibleHeight + value.deltaY < 0) {
      ...
      this.reviseOffset = this.invisibleHeight % this.LINE_HEIGHT;
    } else {
      this.invisibleHeight += value.deltaY;
      this.visibleListStart = Math.floor(this.invisibleHeight / this.LINE_HEIGHT);
  ...
  mounted() {
    ...
    this.$bus.$on(BUS_UI.SYNC_RESIZE, () => {
      this.visibleHeight = (this.$refs.scrollList as HTMLElement).clientHeight;
    });
    ...
    const mouseMoveHandler = (e: MouseEvent) => {
      ...
      if (scrolled + offset < 0) {
        this.invisibleHeight = 0;
      } else if (scrolled + offset > this.visibleHeight - this.thumbHeight) {
        this.invisibleHeight = this.visibleHeight - this.thumbHeight;
      } else {
        scrolled += offset;
        this.invisibleHeight += scrolled * (this.visibleHeight / this.logicalHeight);
      }
    };

    const mouseDownHandler = (e: MouseEvent) => {
      startY = e.clientY;
      scrolled = this.scrollBarTop;
    ...
    const mouseUpHandler = (e: MouseEvent) => {
      ...
      if (scrolled <= this.visibleHeight - this.thumbHeight) {
        this.invisibleHeight += scrolled * (this.visibleHeight / this.logicalHeight);
      }
    };
    */
    ...
    setTimeout(() => {
      ...
      // (this.$refs.thumb as HTMLElement).addEventListener(
      //   "mousedown",
      //   mouseDownHandler,
      //   false
      // );
