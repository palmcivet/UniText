        ...
        <a-tab-pane :tab="group" v-for="(group, index) in groups" :key="index + 1">
          <div v-for="(item, index1) in currentThemeConfig" :key="index1">
            ...
            >
              <!-- 普通输入 -->
              <a-input
                v-if="item.type === 'input' && !item.card"
                :placeholder="item.note"
                v-model="form[item.name]"
              />

              <!-- 带颜色卡片输入 -->
              <a-popover title="Color" trigger="click" placement="bottomLeft">
                <color-card
                  slot="content"
                  @change="handleColorChange($event, item.name)"
                />
                <a-input
                  :ref="`color${index1}`"
                  v-if="item.type === 'input' && item.card === 'color'"
                  :placeholder="item.note"
                  v-model="form[item.name]"
                >
                  <div
                    ...
                    slot="prefix"
                    v-if="form[item.name]"
                    :style="{ backgroundColor: form[item.name] }"
                  ></div>
                </a-input>
              </a-popover>

              <!-- 带文章卡片输入 -->
              <a-popover title="文章" trigger="click" placement="bottomLeft">
                <posts-card
                  slot="content"
                  :posts="postsWithLink"
                  @select="handlePostSelected($event, item.name)"
                ></posts-card>
                <a-input
                  :ref="`color${index1}`"
                  v-if="item.type === 'input' && item.card === 'post'"
                  :placeholder="item.note"
                  v-model="form[item.name]"
                />
              </a-popover>
              <div
                class="tip-text mb-1"
                v-if="item.type === 'input' && item.card === 'post' && form[item.name]"
              >
                ...
                >
                  <div v-if="form[item.name] && form[item.name].startsWith('/media/')">
                    <img
                      class="picture"
                      :src="
                        `file://${site.appDir}/themes/${
                          site.themeConfig.themeName
                        }/public${form[item.name]}`
                      "
                      height="150"
                    />
                  </div>
                  <div v-else-if="form[item.name] === ''">
                    <img src="/public/images/image_upload.svg" class="picture" />
              ...
              <div v-if="item.type === 'array'">
                <div
                  class="item-card"
                  v-for="(configItem, configItemIndex) in form[item.name]"
                  :key="configItemIndex"
                >
                  <div v-for="(field, fieldIndex) in item.arrayItems" :key="fieldIndex">
                    ...
                    >
                      <!-- 普通输入 -->
                      <a-input
                        v-if="field.type === 'input' && !field.card"
                        :placeholder="field.note"
                        v-model="configItem[field.name]"
                      />
                      ...
                      <a-popover title="Color" trigger="click" placement="bottomLeft">
                        ...
                        ></color-card>
                        <a-input
                          :ref="`color-${configItemIndex}-${fieldIndex}`"
                          v-if="field.type === 'input' && field.card === 'color'"
                          :placeholder="field.note"
                          v-model="configItem[field.name]"
                        >
                          <div
                            ...
                            :style="{
                              backgroundColor: configItem[field.name],
                            }"
                          ></div>
                        </a-input>
                      </a-popover>
                      ...
                      <a-popover title="文章" trigger="click" placement="bottomLeft">
                        ...
                        <a-input
                          :ref="`color-${configItemIndex}-${fieldIndex}`"
                          v-if="field.type === 'input' && field.card === 'post'"
                          :placeholder="field.note"
                          v-model="configItem[field.name]"
                        />
                      </a-popover>
                      ...
                      >
                        <a-select-option
                          v-for="(option, index2) in field.options"
                          :key="index2"
                          :value="option.value"
                        >
                          {{ option.label }}
                      ...
                      <div v-if="field.type === 'picture-upload'" style="display: flex;">
                        ...
                        >
                          <div
                            v-if="
                              configItem[field.name] &&
                                configItem[field.name].startsWith('/media/')
                            "
                          >
                            <img
                              class="picture"
                          ...
                          <div v-else>
                            <img
                              class="picture"
                              :src="`file://${configItem[field.name]}`"
                              alt
                            />
                          </div>
                        </a-upload>
                        <a-tooltip placement="left" title="Reset">
                          <a-button
                            ...
                            shape="circle"
                            site="small"
                            @click="resetFormItem(item.name, field.name, configItemIndex)"
                          >
                            <i class="zwicon-undo"></i>
                          </a-button>
                        </a-tooltip>
                  ...
                  <a-button
                    shape="circle"
                    @click="
                      addConfigItem(form[item.name], configItemIndex, item.arrayItems)
                    "
                  ...
                  <a-button
                    shape="circle"
                    @click="deleteConfigItem(form[item.name], configItemIndex)"
                  >
                ...
                <a-button
                  ...
                  block
                  @click="addConfigItem(form[item.name], 0, item.arrayItems)"
                >
                  <i class="zwicon-plus"></i>
                </a-button>
              </div>
...
export default class ThemeCustomSetting extends Vue {
  ...
  get postsWithLink() {
    const list = this.site.posts
      .map((post: any) => {
        return {
          ...post,
          link: urlJoin(
            this.site.setting.domain,
            this.site.themeConfig.postPath,
            post.fileName,
            "/"
          ),
        };
  ...
  resetThemeCustomConfig() {
    this.$confirm({
      ...
      onOk: () => {
        ...
        ipcRenderer.once(
          "theme-custom-config-saved",
          async (event: IpcRendererEvent, result: any) => {
            await this.$bus.$emit("site-reload");
            this.$router.push({
              name: "loading",
              query: { redirect: "theme?tab=custom" },
            });
  ...
  resetFormItem(
    ...
    const originalItem = this.currentThemeConfig.find(
      (item: any) => item.name === formItemName
    );
    if (arrayFieldItemName && typeof configItemIndex === "number") {
      const foundItem = originalItem.arrayItems.find(
        (item: any) => item.name === arrayFieldItemName
      );
