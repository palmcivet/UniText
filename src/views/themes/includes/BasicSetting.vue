  ...
  <div>
    <ValidationObserver ref="observer" v-slot="{ invalid }">
      <a-form :form="form" style="padding-bottom: 48px;">
          ...
          <a-select v-model="form.themeName" optionLabelProp="name">
            ...
            >
              <div class="theme-option">
                ...
                <div class="extra" v-if="item.repository">
                  <a-button
                    @click.stop="openPage(item.repository)"
                    type="dashed"
                    shape="circle"
                    size="small"
                    icon="github"
            ...
            <div slot="dropdownRender" slot-scope="menu">
              ...
              <div
                ...
                @click="openPage('https://gridea.dev/themes/')"
              >
        ...
        <a-form-item
          :label="$t('articleDefault')"
          :labelCol="formLayout.label"
          :wrapperCol="formLayout.wrapper"
          :colon="false"
        >
          <a-radio-group name="postUrlFormat" v-model="form.postUrlFormat">
            <a-radio v-for="item in urlFormats" :key="item.value" :value="item.value">
              {{ item.text }}
            </a-radio>
          </a-radio-group>
        </a-form-item>
        ...
        >
          ...
          <div>
            <a @click.prevent="openPage('http://momentjs.cn/docs/#/displaying/format/')">
              Momentjs Format
            </a>
          </div>
        </a-form-item>
...
export default class ThemeBasicSetting extends Vue {
  ...
  saveTheme() {
    ...
    ipcRenderer.once("theme-saved", async (event: IpcRendererEvent, result: any) => {
      ...
      this.$router.push({
        name: "loading",
        query: { redirect: "theme?tab=basic" },
      });
