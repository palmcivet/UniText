...
export default class Logger {
  ...
  captureWindowLog(window: BrowserWindow, name?: string) {
    const windowName = name ?? window.webContents.id.toString();
    if (!this.logDirectory) {
      this.warn(
        `Cannot capture window log for window ${windowName}. Please initialize the logger to set logger directory!`
      );
      return;
    }
    const loggerPath = resolve(this.logDirectory, `renderer.${windowName}.log`);
    this.log(`Setup renderer logger for window ${windowName} to ${loggerPath}`);
